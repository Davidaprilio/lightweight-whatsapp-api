...
let dirSession = './session/legacy/david_store.json';
...
const startSock = async() => {
	...
	sock.ev.on('connection.update', (update) => {
		...
		console.log('connection update', lastDisconnect.error.message)
    if (lastDisconnect.error.message === "closed connection to WhatsApp") {
      if (fs.existsSync(dirSession)) {
        fs.rmSync(dirSession, { recursive: true, force: true });
      }
    }
	})
